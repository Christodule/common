...
export const fetchUser = () => (dispatch) => {
  ...
  onAuthStateChanged(auth, user => {
    if (user) {
      ...
      const languageData = store.getState().languagedata;
      if (languageData && languageData.defaultLanguage) {
        dispatch({
          type: FETCH_USER_FAILED,
          payload: { code: languageData.defaultLanguage.auth_error, message: languageData.defaultLanguage.not_logged_in }
        });
      }
    }
  });
